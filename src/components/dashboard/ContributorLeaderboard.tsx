...
export const ContributorLeaderboard: React.FC<ContributorLeaderboardProps> = ({
  ...
  return (
    <div className="rounded-2xl bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-5 flex flex-col h-full shadow-xs">
            ...
            <div
              ...
              className={`w-full p-2.5 rounded-xl border transition-colors flex items-center justify-between gap-3 ${
                isSelected
                  ...
                  : 'bg-slate-50 dark:bg-slate-950/50 border-slate-200 dark:border-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
              <button
                type="button"
                ...
                className="flex-1 min-w-0 text-left flex items-center justify-between gap-3 cursor-pointer focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-cyan-500 rounded-lg p-0.5 -m-0.5"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-xs font-mono tabular-nums font-medium text-slate-500 dark:text-slate-400 w-5 text-right">
                    #{index + 1}
                  </span>
                  <img
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-slate-900 dark:text-slate-200 truncate flex items-center gap-1.5">
                      <span>{contributor.login}</span>
                      {index === 0 && (
                        <span className="text-[10px] px-1 py-0.2 rounded-sm bg-amber-500/20 text-amber-700 dark:text-amber-300 font-normal">
                          Lead
                        </span>
                      )}
                    </div>
                    <div className="w-24 bg-slate-200 dark:bg-slate-800 h-1 rounded-full mt-1.5 overflow-hidden">
                      <div
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg hover:bg-slate-200/60 dark:hover:bg-slate-800 transition-colors focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-cyan-500 shrink-0"
