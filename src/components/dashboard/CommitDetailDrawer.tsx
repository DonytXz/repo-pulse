  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-xl h-full bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col text-slate-200 overflow-hidden animate-in slide-in-from-right duration-250"
      >
        {/* Drawer Header */}
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h2 id="commit-drawer-title" className="text-sm font-semibold text-white truncate flex items-center gap-2">
                <span>Commit</span>
                <span className="font-mono text-cyan-400 text-xs px-1.5 py-0.5 rounded-md bg-cyan-500/10 border border-cyan-500/20">
                  {shortSha}
                </span>
              </h2>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-slate-200 rounded-lg hover:bg-slate-800 transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-cyan-400"
              title="Copy SHA"
            >
              {copied ? (
            ...
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-cyan-400"
            >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 text-xs">
          ...
          {error && !isLoading && (
            <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300">
              ...
              <p className="text-[11px] text-slate-400">{error instanceof Error ? error.message : 'Unknown error'}</p>
            </div>
          )}
            ...
            <>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    {commitDetail.author?.avatar_url ? (
                      ...
                      <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-mono text-slate-300">
                  ...
                  {commitDetail.commit.verification?.verified && (
                    <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium text-[10px]">
                  ...
                  <div className="pt-2 border-t border-slate-800/60 flex items-center gap-2 text-[11px] text-slate-400">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {commitDetail.parents.map((p) => (
                        <span key={p.sha} className="font-mono text-cyan-400 bg-slate-900 px-1.5 py-0.5 rounded-md border border-slate-800">
                          {p.sha.slice(0, 7)}
                        </span>
                      ))}
                    </div>
              ...
              {commitDetail.stats && (
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-emerald-400">
                  ...
                  <div className="p-3 rounded-xl bg-rose-500/5 border border-rose-500/20 text-rose-400">
                    ...
                    {commitDetail.files.map((file) => (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="font-mono text-[11px] text-slate-200 truncate" title={file.filename}>
                            {file.filename}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 shrink-0 font-mono text-[10px]">
                          ...
                          <span className="text-emerald-400 tabular-nums">+{file.additions}</span>
                          <span className="text-rose-400 tabular-nums">-{file.deletions}</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/60 flex items-center justify-between">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-3.5 py-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-semibold text-xs flex items-center gap-1.5 transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-cyan-300 shadow-md shadow-cyan-500/20"
          >
